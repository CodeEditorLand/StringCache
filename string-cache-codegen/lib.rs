...
//! The macro can be used in expression or patterns, with strings listed in
//! `build.rs`. For example:
//!
//! ```ignore
...
impl AtomType {
	...
	/// e.g. `"FooAtom"` at the crate root or `"foo::Atom"` if the generated
	/// code is included in a `foo` module.
	///
	...
	///
	/// Note that `docs` should not contain the `///` at the front of normal
	/// docs.
	...
	///
	/// This can help the user know that this type is zero-sized and just
	/// references a static lookup table, or point them to the `Atom` type
	/// alias for more info.
	///
	/// Note that `docs` should not contain the `///` at the front of normal
	/// docs.
	...
	fn to_tokens(&mut self) -> proc_macro2::TokenStream {
		// `impl Default for Atom` requires the empty string to be in the static
		// set. This also makes sure the set in non-empty,
	...
	///
	/// Typical usage:
	/// `.write_to_file(&Path::new(&env::var("OUT_DIR").unwrap()).join("
	/// foo_atom.rs"))`
