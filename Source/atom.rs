...
use crate::{
	dynamic_set::{Entry, DYNAMIC_SET},
	static_sets::StaticAtomSet,
};
...
///
/// While the type definition for `Atom` indicates that it generic on a
/// particular implementation of an atom set, you don't need to worry about
/// this.  Atoms can be static and come from a `StaticAtomSet` generated by the
/// `string_cache_codegen` crate, or they can be dynamic and created by you on
/// an `EmptyStaticAtomSet`.
///
/// `Atom` implements `Clone` but not `Copy`, since internally atoms are
/// reference-counted; this means that you may need to `.clone()` an atom to
/// keep copies to it in different places, or when passing it to a function that
/// takes an `Atom` rather than an `&Atom`.
///
/// ## Creating an atom at runtime
///
/// If you use `string_cache_codegen` to generate a precomputed list of atoms,
/// your code may then do something like read data from somewhere and extract
/// tokens that need to be compared to the atoms.  In this case, you can use
/// `Atom::from(&str)` or `Atom::from(String)`.  These create a
/// reference-counted atom which will be automatically freed when all references
/// to it are dropped.
///
/// This means that your application can safely have a loop which tokenizes
/// data, creates atoms from the tokens, and compares the atoms to a predefined
/// set of keywords, without running the risk of arbitrary memory consumption
/// from creating large numbers of atoms — as long as your application does not
/// store clones of the atoms it creates along the way.
///
/// For example, the following is safe and will not consume arbitrary amounts of
/// memory:
///
/// ```ignore
...
/// ```
#[derive(PartialEq, Eq)]
// NOTE: Deriving PartialEq requires that a given string must always be interned
// the same way.
...
// FIXME: bound removed from the struct definition before of this error for
// pack_static: "error[E0723]: trait bounds other than `Sized` on const fn
// parameters are unstable" https://github.com/rust-lang/rust/issues/57563
...
impl<Static:StaticAtomSet> Atom<Static> {
	...
	#[doc(hidden)]
	pub fn unsafe_data(&self) -> u64 { self.unsafe_data.get() }

	/// Return true if this is a static Atom. For testing.
	#[doc(hidden)]
	pub fn is_static(&self) -> bool { self.tag() == STATIC_TAG }

	/// Return true if this is a dynamic Atom. For testing.
	#[doc(hidden)]
	pub fn is_dynamic(&self) -> bool { self.tag() == DYNAMIC_TAG }
...
impl<Static:StaticAtomSet> Default for Atom<Static> {
	#[inline]
	fn default() -> Self { Atom::pack_static(Static::empty_string_index()) }
}
...
// AsciiExt requires mutating methods, so we just implement the non-mutating
// ones. We don't need to implement is_ascii because there's no performance
// improvement over the one from &str.
impl<Static:StaticAtomSet> Atom<Static> {
