		...
		let bucket_index = (hash & BUCKET_MASK) as usize;
		{
			...
			while let Some(entry) = ptr.take() {
				if entry.hash == hash && *entry.string == *string {
					...
					// Uh-oh. The pointer's reference count was zero, which
					// means someone may try to free it. (Naive attempts to
					// defend against this, for example having the
					// destructor check to see whether the reference count is
					// indeed zero, don't work due to ABA.) Thus we need to
					// temporarily add a duplicate string to the list.
